...
title: 新项目操作手册
type: concept
tags: [agent-dev, playbook, new-project, sdd]
created: 2026-05-24
updated: 2026-05-24
sources:
...
规格必须有验收标准，不是模糊 prompt。验收标准是 agent 和人类的契约。
...
多个 agent 并行，用看门狗监控进度。10 个任务可从 30h 缩短到 4h。

### 5. 对照验收标准审查

审查验收标准是否满足，不是逐行 code review。人类角色从写代码转变为写规格和审查结果。
