...
> 配置备份已同步到 GitHub 私有仓库，详见 [[Termux 配置备份]]
>
> 仓库: `git@github.com:OuYang-HX/termux-backup.git`（Private）
> 本地: `~/termux-backup/`

备份仅包含配置文件（bashrc、termux.properties、ssh-config、pi-settings、herdr-config），不含包列表和脚本。
...
| Pi API Keys | 敏感信息 | 手动编辑 `models.json` / `auth.json` |
| Pi agents/chains/extensions | 安装自动生成 | `pi install npm:gentle-pi` 等 |
| 项目代码 | 仓库克隆 | git clone |

### 快速恢复

```bash
# 1. 安装 Termux (F-Droid) + pkg install git openssh
# 2. ssh-keygen → 加到 GitHub
# 3. git clone git@github.com:OuYang-HX/termux-backup.git ~/termux-backup
# 4. 复制 dotfiles/ 到对应位置（见仓库 README）
# 5. 安装字体 + termux-reload-settings
# 6. 手动填入 API Keys 和 SSH 私钥
```
